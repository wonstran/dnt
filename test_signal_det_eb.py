...
input_video = '/mnt/d/videos/ped2stage/05032024/eb_20240417.mp4'
...
detector = SignalDetector(det_zones=[(330, 55, 30, 30)], batchsz=1024, weights='/mnt/d/videos/ped2stage/05032024/signal/eb_ped_signal.pt')
dets = detector.detect(input_video=input_video, det_file=det_file)
sigs = detector.gen_ped_interval(dets=dets, input_video=input_video, walk_interval=7, countdown_interval=10, out_file=sig_file)
labels = detector.generate_labels(signals=sigs, input_video=input_video, label_file=label_file, size_factor=1.2, thick=2)

labeler = Labeler()
labeler.draw(input_video=input_video, output_video=label_video, draw_file=label_file, start_frame=8*60*60*25, end_frame=9*60*60*25)
